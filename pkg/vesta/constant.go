...
func GetDefaultExecAllocatorOptions() []chromedp.ExecAllocatorOption {
	...
	cwd, err := os.Getwd()
	if err != nil {
		log.Fatalf("获取当前工作目录失败: %v, cwd: %v", err, cwd)
	}
	userDataDir = filepath.Join(cwd, "vesta-user-data") //考虑采用浏览器的默认位置，
	return append(chromedp.DefaultExecAllocatorOptions[:],
		chromedp.IgnoreCertErrors,                    // 忽略错误，这样可以直接跳过https的认证错误页面)
		chromedp.Flag("enable-automation", false),    // false 禁用自动化提示
		chromedp.Flag("disable-web-security", false), // 禁用网络安全，使得可以跨域操作iframe
